...
export default function ImageManager({
  ...
  function seleccionarImagenes(
    ...
    if (formatoIncorrecto) {
      alert(
        `El archivo "${formatoIncorrecto.name}" no es JPG, PNG o WEBP.`
      );
      return;
    }
    ...
    if (images.length + nuevosArchivos.length > maxImages) {
      alert(
        `Podés cargar como máximo ${maxImages} imágenes por vehículo.`
      );
      return;
    }
  ...
  function eliminarImagen(index: number) {
    const nuevasImagenes = images.filter(
    ...
    >
      <div>
        ...
        <small style={{ color: "#4b5563" }}>
          La primera imagen será la portada. Máximo {maxImages} fotos y
          10 MB por imagen.
        </small>
      </div>
      ...
      <button
        ...
        style={{
          ...
          borderRadius: 8,
          backgroundColor:
            images.length >= maxImages ? "#d1d5db" : "#111827",
          color: "#ffffff",
          fontWeight: 700,
          cursor:
            images.length >= maxImages ? "not-allowed" : "pointer",
        }}
      >
        Agregar imágenes
      </button>
